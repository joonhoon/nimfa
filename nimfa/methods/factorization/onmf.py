...
"""
###################################
Onmf (``methods.factorization.onmf``)
###################################

**Orthogonal Nonnegative Matrix Factorization using multiplicative updates
directly from the true gradient on Stiefel manifold (ONMF)** [Choi2008]_.

In orthogonal NMF, orthogonality constraints are imposed on basis matrix or
mixture matrix. Based on Euclidean distance, it uses multiplicative updates
directly from the true gradient (natural gradient) in Stiefel manifold,
whereas existing algorithms consider additive orthogonality constraints. 

.. literalinclude:: /code/snippet_onmf_basis.py

.. literalinclude:: /code/snippet_onmf_mixture.py
